...
| shared checkpoint state | durable writes | Compact `checkpoint_*` facts written to explicit entity targets so other agents can resume shared work without inheriting private attendant state. |
...
| project policy appendix | bounded brief appendix | Optional handshake/reconvene appendix loaded from explicit project-memory policy facts so repo-scoped preferences can travel with the brief without becoming global system rules. |
...
12. After any action that changed knowledge state, persist the result through `write()` and/or `checkpoint()` instead of waiting only for the end of the turn.
...
14. When that new finding should change what is loaded next, call `attend()` again before continuing.
...
20. `checkpoint()` remains agent-scoped for private session recovery, but when `entityTargets` are supplied it also writes shared checkpoint state:
   - `checkpoint_summary`
   - `current_step`
   - `next_step`
   - `open_risks`
21. When `checkpoint().actions` is supplied alongside `entityTargets`, Iranti should also append those structured activity records into the shared entity's canonical `recent_actions` fact so later sessions can see which commands, validations, searches, and decisions already happened.
...
23. Shared checkpoint state should not replace the canonical project facts such as `next_step` or `decision`; they are resumability hints, not the sole source of truth.
...
30. Under-logged runs are non-compliant for first-party Iranti hosts. When applicable, the reminder should name the missing write categories explicitly, such as what was found, what worked, what failed, what changed, and what happens next.
...
32. `attend()` responses should also carry explicit usage guidance that reinforces the required call loop: handshake at session start, attend before each reply, attend before and after knowledge discovery, call iranti_write after knowledge-changing actions, then attend again when those writes should change what is loaded next.
...
37. Freshness routing may expand one relationship hop beyond the directly watched entities, so a watched project can refresh on a fresh write to a directly related issue/task/entity without requiring the host to restate both scopes.
...
39. Handshake and reconvene may also load a bounded set of project-scoped policy facts from the configured `IRANTI_MEMORY_ENTITY` and return them as `projectPolicies`.
40. Project-scoped policy loading should be deterministic and narrow: prefer explicit keys such as `agent_operating_rules`, `agent_preferences`, and `*_rule`/`*_rules`/`*_preference`/`*_preferences`, plus explicit preference metadata written by Iranti.
41. Loaded project policies may be appended to the effective operating rules as `PROJECT POLICY (...)` guidance and may also appear in working memory as compact policy entries so the host can see what shaped the brief.
42. Project policies are advisory project behavior, not a new trust surface; they must not override protected Staff Namespace rules or first-party protocol enforcement.
...
- Shared checkpoint state is demoted below canonical task facts during observe/attend selection so checkpoints help recovery without crowding out the main `next_step`, `blocker`, or artifact facts.
- Structured `recent_actions` records should stay compact and recovery-oriented. They are for material commands, tests, validations, searches, or decisions that later sessions would otherwise rerun blindly, not for every shell keystroke.
...
- Project policy loading is intentionally bounded and explicit. It should not scrape arbitrary project memory into operating rules just because a fact happens to mention preferences or process.
...
- `tests/cross-tool/run_cross_tool_handoff_tests.ts` verifies shared checkpoint state is written to explicit entity targets and remains secondary to canonical shared task facts during observe/attend retrieval.
...
- `tests/session-recovery/run_session_recovery_tests.ts` also verifies handshake surfaces configured project-scoped policies on the brief, appends them to the effective operating rules, and exposes them as working-memory entries.
...
- `tests/api-surfaces/run_protocol_enforcement_route_tests.ts` verifies strict route-level handshake responses surface `projectPolicies` and append configured project-scoped policy text to the operating rules.
- `tests/mcp/run_protocol_enforcement_regressions.ts` verifies the strict MCP handshake path surfaces `projectPolicies` and appends configured project-scoped policy text to the operating rules.
- `scripts/test-attendant.ts`, `scripts/test-sdk.ts`, and `tests/mcp/smoke_test.ts` exercise the bounded project-policy path in smoke coverage so first-party surfaces keep returning the enriched handshake brief.
