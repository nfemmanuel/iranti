...
4. Ask for the instance name, port, and a non-placeholder `DATABASE_URL`.
5. Ask for the default LLM provider.
6. If the provider is remote, require its API key.
7. Offer to collect additional provider keys for other supported providers.
8. Generate or rotate a usable instance `IRANTI_API_KEY` so the instance can run even without DB-backed registry setup.
9. Create or update the target instance env.
10. Offer to bind one or more project folders by writing `.env.iranti`.
11. For each bound project, optionally scaffold Claude Code MCP and hook files.
12. If Codex is installed and at least one project was bound, optionally register Codex globally against the first bound project.
13. Print a runnable summary with next-step commands.
