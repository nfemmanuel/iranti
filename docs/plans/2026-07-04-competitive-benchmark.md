...
- [~] S2. The fair competitive harness — PRD `bench-1-fair-competitive-harness.md` DRAFTING now (prd-fair-harness agent). RECON DONE (`project/iranti/research/local-benchmark-assets/s3_recon_reuse_map`): NO reusable LoCoMo/LongMemEval/LLM-judge exists anywhere — must build the real protocol (iranti-benchmarking's own audit warns against fake "loosely-motivated-by" replications). REUSE: ai-mem's `bench/mcpclient.mjs` MCP client + iranti-benchmarking's Mem0 python adapter. Dual-track G/H lives here.
