...
- **RULE-1** ◐ **PARTIALLY FIXED (2026-07-03)** — Tokenizer short-token floor. Digit-bearing 2-char tokens (S3, R2, A1) now survive `tokenizeMessage`'s length floor (function words never contain digits — zero stop-word noise risk; bench re-run confirmed no metric moved). **Residual:** pure-alpha 2-char acronyms (PR, UI, CI) are still dropped — admitting them wholesale would admit every un-stopworded preposition; closing that class needs a different mechanism (e.g. case-aware tokenization or a curated allowlist), each a measured change against the full bench.
