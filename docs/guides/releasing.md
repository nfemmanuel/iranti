...
Current repo version is `0.2.30`. If the next release is `0.2.31`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.31
```
...
gh release create v0.2.31 --title "v0.2.31" --notes "Release notes here"
```
