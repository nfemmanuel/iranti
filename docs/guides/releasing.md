...
Current repo version is `0.2.13`. If the next release is `0.2.14`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.14
```
...
gh release create v0.2.14 --title "v0.2.14" --notes "Release notes here"
```
