...
Current repo version is `0.3.19`. If the next release is `0.3.20`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.20
```
...
gh release create v0.3.20 --title "v0.3.20" --notes "Release notes here"
```
