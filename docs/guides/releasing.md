...
Current repo version is `0.2.26`. If the next release is `0.2.27`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.27
```
...
gh release create v0.2.27 --title "v0.2.27" --notes "Release notes here"
```
