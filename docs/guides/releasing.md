...
Current repo version is `0.2.25`. If the next release is `0.2.26`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.26
```
...
gh release create v0.2.26 --title "v0.2.26" --notes "Release notes here"
```
