...
Current repo version is `0.2.51`. If the next release is `0.2.52`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.52
```
...
gh release create v0.2.52 --title "v0.2.52" --notes "Release notes here"
```
