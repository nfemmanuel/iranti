...
Current repo version is `0.3.20`. If the next release is `0.3.21`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.21
```
...
gh release create v0.3.21 --title "v0.3.21" --notes "Release notes here"
```
