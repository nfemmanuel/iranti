...
Current repo version is `0.2.32`. If the next release is `0.2.33`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.33
```
...
gh release create v0.2.33 --title "v0.2.33" --notes "Release notes here"
```
