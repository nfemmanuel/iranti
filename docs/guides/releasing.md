...
Current repo version is `0.2.8`. If the next release is `0.2.9`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.9
```
...
gh release create v0.2.9 --title "v0.2.9" --notes "Release notes here"
```
