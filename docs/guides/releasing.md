...
Current repo version is `0.2.41`. If the next release is `0.2.42`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.42
```
...
gh release create v0.2.42 --title "v0.2.42" --notes "Release notes here"
```
