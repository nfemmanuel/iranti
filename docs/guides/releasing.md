...
Current repo version is `0.2.23`. If the next release is `0.2.24`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.24
```
...
gh release create v0.2.24 --title "v0.2.24" --notes "Release notes here"
```
