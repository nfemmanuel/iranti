...
Current repo version is `0.2.48`. If the next release is `0.2.49`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.49
```
...
gh release create v0.2.49 --title "v0.2.49" --notes "Release notes here"
```
