...
Current repo version is `0.3.41`. If the next release is `0.3.42`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.42
```
...
gh release create v0.3.42 --title "v0.3.42" --notes "Release notes here"
```
