...
Current repo version is `0.3.7`. If the next release is `0.3.8`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.8
```
...
gh release create v0.3.8 --title "v0.3.8" --notes "Release notes here"
```
