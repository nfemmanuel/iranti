...
Current repo version is `0.2.2`. If the next release is `0.2.3`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.3
```
...
gh release create v0.2.3 --title "v0.2.3" --notes "Release notes here"
```
