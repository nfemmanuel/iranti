...
Current repo version is `0.2.10`. If the next release is `0.2.11`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.11
```
...
gh release create v0.2.11 --title "v0.2.11" --notes "Release notes here"
```
