...
Current repo version is `0.3.38`. If the next release is `0.3.39`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.39
```
...
```bash
git tag v0.3.39
git push origin v0.3.39
gh release create v0.3.39 --title "v0.3.39" --notes "Release notes here"
```
