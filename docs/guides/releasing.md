...
Current repo version is `0.2.16`. If the next release is `0.2.17`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.17
```
...
gh release create v0.2.17 --title "v0.2.17" --notes "Release notes here"
```
