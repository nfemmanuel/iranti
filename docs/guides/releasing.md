...
Current repo version is `0.2.14`. If the next release is `0.2.15`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.15
```
...
gh release create v0.2.15 --title "v0.2.15" --notes "Release notes here"
```
