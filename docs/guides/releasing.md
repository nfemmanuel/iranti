...
Current repo version is `0.2.22`. If the next release is `0.2.23`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.23
```
...
gh release create v0.2.23 --title "v0.2.23" --notes "Release notes here"
```
