...
Current repo version is `0.3.17`. If the next release is `0.3.18`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.18
```
...
gh release create v0.3.18 --title "v0.3.18" --notes "Release notes here"
```
