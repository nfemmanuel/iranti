...
Current repo version is `0.3.35`. If the next release is `0.3.36`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.36
```
...
gh release create v0.3.36 --title "v0.3.36" --notes "Release notes here"
```
