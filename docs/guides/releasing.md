...
Current repo version is `0.3.1`. If the next release is `0.3.2`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.2
```
...
gh release create v0.3.2 --title "v0.3.2" --notes "Release notes here"
```
