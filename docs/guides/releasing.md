...
Current repo version is `0.2.46`. If the next release is `0.2.47`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.47
```
...
gh release create v0.2.47 --title "v0.2.47" --notes "Release notes here"
```
