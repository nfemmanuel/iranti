...
Current repo version is `0.2.29`. If the next release is `0.2.30`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.30
```
...
gh release create v0.2.30 --title "v0.2.30" --notes "Release notes here"
```
