...
Current repo version is `0.3.18`. If the next release is `0.3.19`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.19
```
...
gh release create v0.3.19 --title "v0.3.19" --notes "Release notes here"
```
