...
Current repo version is `0.3.5`. If the next release is `0.3.6`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.6
```
...
gh release create v0.3.6 --title "v0.3.6" --notes "Release notes here"
```
