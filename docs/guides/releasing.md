...
Current repo version is `0.2.28`. If the next release is `0.2.29`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.29
```
...
gh release create v0.2.29 --title "v0.2.29" --notes "Release notes here"
```
