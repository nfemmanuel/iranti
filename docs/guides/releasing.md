...
Current repo version is `0.2.6`. If the next release is `0.2.7`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.7
```
...
gh release create v0.2.7 --title "v0.2.7" --notes "Release notes here"
```
