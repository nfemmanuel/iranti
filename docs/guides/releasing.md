...
Current repo version is `0.2.33`. If the next release is `0.2.34`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.34
```
...
gh release create v0.2.34 --title "v0.2.34" --notes "Release notes here"
```
