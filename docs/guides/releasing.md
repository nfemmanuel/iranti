...
Current repo version is `0.2.47`. If the next release is `0.2.48`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.48
```
...
gh release create v0.2.48 --title "v0.2.48" --notes "Release notes here"
```
