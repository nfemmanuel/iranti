...
Current repo version is `0.2.15`. If the next release is `0.2.16`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.16
```
...
gh release create v0.2.16 --title "v0.2.16" --notes "Release notes here"
```
