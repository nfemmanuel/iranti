...
Current repo version is `0.2.44`. If the next release is `0.2.45`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.45
```
...
gh release create v0.2.45 --title "v0.2.45" --notes "Release notes here"
```
