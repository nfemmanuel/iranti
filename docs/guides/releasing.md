...
Current repo version is `0.2.37`. If the next release is `0.2.38`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.38
```
...
gh release create v0.2.38 --title "v0.2.38" --notes "Release notes here"
```
