...
Current repo version is `0.2.38`. If the next release is `0.2.39`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.39
```
...
gh release create v0.2.39 --title "v0.2.39" --notes "Release notes here"
```
