...
Current repo version is `0.3.36`. If the next release is `0.3.37`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.3.37
```
...
gh release create v0.3.37 --title "v0.3.37" --notes "Release notes here"
```
