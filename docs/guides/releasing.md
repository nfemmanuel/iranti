...
Current repo version is `0.2.19`. If the next release is `0.2.20`, use the following exact sequence.

1. Bump versions in one step:

```bash
npm run release:bump -- 0.2.20
```
...
gh release create v0.2.20 --title "v0.2.20" --notes "Release notes here"
```
