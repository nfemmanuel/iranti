...
TASK_INFERENCE_MODEL=gemini-2.5-flash
...
**Available models:**
- `gpt-5-mini` — Fast, cheap, good for most tasks
- `gpt-5` — More capable, more expensive
- `gpt-5-nano` — Lowest-cost GPT-5 tier for lightweight tasks
...
RELEVANCE_MODEL=gpt-5-mini
SUMMARIZATION_MODEL=gpt-5-mini
TASK_INFERENCE_MODEL=gpt-5-mini
EXTRACTION_MODEL=gpt-5-mini

# Optional: use full gpt-5 for harder reasoning
CONFLICT_MODEL=gpt-5
```
...
**Cost:** ~$0.25 per 1M input tokens (gpt-5-mini), ~$1.25 per 1M input tokens (gpt-5)
...
**Available models:**
- `meta-llama/llama-4-scout-17b-16e-instruct` � Current default balance
...
| `conflict_resolution` | Reason about contradicting facts | Capable model (pro, gpt-5) |
...
Task overrides must be compatible with the selected provider.
Example mismatch: `LLM_PROVIDER=openai` with `CLASSIFICATION_MODEL=gemini-2.5-flash`.

Iranti now ignores incompatible overrides and falls back to a sane provider default
for that task, with a warning log so you can clean up stale env values.
...
- **OpenAI GPT-5** — If you need the best reasoning
...
| OpenAI | gpt-5-mini | $0.25 | Fast | Good |
| Mistral | small | $0.10 | Fast | Good |
| Gemini | 2.5-pro | $1.25 | Medium | Excellent |
| OpenAI | gpt-5 | $1.25 | Medium | Excellent |
