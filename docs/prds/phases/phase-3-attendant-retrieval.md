...
- [ ] A session with extractable signal and zero explicit writes still accumulates facts (backstop); nothing blocks at default enforcement. **[CORE-32 RE-SCOPED 2026-06-28]** The non-blocking auto-write-at-reduced-confidence path IS shipped and is the intended "born inverted / never block the host" behavior. The `IRANTI_ENFORCE=true` blocking branch is DEFERRED — `IRANTI_ENFORCE` has zero occurrences in `src/` (verified by grep); blocking a host requires per-turn write tracking that `attend` does not have, and is off by default by design. The shipped + intended behavior is: auto-write with breadcrumb warning; blocking enforcement deferred.
...
- [ ] `media_objects` exists and is typed; `metric_counters` is tenant-scoped; embeddings off by default. **[CORE-16 RE-SCOPED 2026-06-28]** Schema scaffolding only shipped: `facts.embedding vector(768)` column + HNSW index (migration 0010), off by default. The hybrid vector+keyword RETRIEVAL PATH is deferred to a future embeddings phase — no code in `src/` reads the embedding column; "hybrid search works when enabled" wording was aspirational and is NOT implemented. The shipped state is: schema pre-placed (embedding column + HNSW index, off by default); read/query path deferred until there is a consumer.
...
- 2026-06-28 — CORE-16 re-scoped: only schema scaffolding shipped (embedding column + HNSW index, off by default); the hybrid vector+keyword retrieval read/query path is deferred to a future embeddings phase — no consumer today, no code in src/ reads the embedding column. "hybrid search works when enabled" criterion was aspirational and does not reflect the shipped state. CORE-32 re-scoped: the non-blocking auto-write (born inverted) is the shipped and intended behavior; the IRANTI_ENFORCE=true hard-blocking branch is deferred — IRANTI_ENFORCE has zero occurrences in src/, and per-turn write tracking needed for blocking is not built.
