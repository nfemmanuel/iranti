...
- Read-side: in `readRelevantFactsWithMatch`'s flow, AFTER exact/keyword scoring, if the deterministic tiers yield fewer than the per-entity cap AND an embedder is active: embed the query, JS-cosine against the in-scope candidate facts' stored vectors, admit hits above a fixed floor into the REMAINING slots only — never displacing a deterministic hit, never granting `matched: true` (a new `semantic: true` label instead, additive response field).
...
- **JS cosine is O(candidates)** per query — fine at current scale; scale trigger documented (D-nongoal).
