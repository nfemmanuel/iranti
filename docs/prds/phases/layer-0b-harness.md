...
- **D4 — Baseline diff, not pass/fail thresholds.** `bench/baseline.json` is checked in; the runner always prints current-vs-baseline per-metric deltas. Setting `UPDATE_BASELINE=1` regenerates it (a `--update-baseline` CLI flag is impossible through vitest, whose argument parser rejects unknown options before the spec loads — found and corrected in review). Why: iranti has no real users yet and today's numbers are known to be low (Layer 0/2b has no entity resolution) — a hardcoded quality floor would either be trivially met (useless) or fail the gate on legitimate zero-feature-change commits (noisy). The diff format is what makes "improved/regressed" concrete without pretending we know what "good" is yet.
...
- [x] `docs/prds/phases/layer-0b-harness.md` (this file) exists, accepted.
- [x] `bench/corpus/*.json` contains 4 persona transcripts (backend-API dev, frontend dev, data/ML person, messy conversationalist), each ~10–15 messages, each with ≥2 correction cases and ≥2 alias references, each with gold facts and probe queries.
- [x] `src/harness/` implements: corpus loader, per-persona fresh-PGlite ingest (through real `attend`/`write`), scorer (recall/precision/hit-rate/confirmation-rate/false-positive-rate), baseline differ, console reporter.
- [x] Each persona corpus includes ≥2 negative (no-answer) probes, and the scorer reports `falsePositiveRate` from them (D3.4).
- [x] `bench/baseline.json` checked in, generated from the current (Layer 0a) main state.
- [x] `pnpm bench` runs green end-to-end, writes `bench/latest.json`, prints a per-persona + overall console table with baseline deltas.
- [x] Determinism: running `pnpm bench` twice produces identical `bench/latest.json` content modulo timestamps (asserted inside `harness.test.ts`, not just manually observed).
- [x] `pnpm typecheck` and `pnpm lint` exit 0.
- [x] Existing suites (`it-runs`, `persistence`, `keys`, and the rest) remain green and untouched.
...
- **Fire-and-forget settle detection is a stability poll, not a completion signal.** attend()'s extraction writes happen in a detached post-response chain with no exposed completion handle, and adding one is out of scope (changing attend internals is a non-goal). The harness polls the store until the fact count is identical across three consecutive 100ms checks (bounded ceiling) before reading results back — strictly stronger than the fixed 300ms sleep it replaced (a review finding: a fixed sleep is calibrated to one machine and threatens the determinism assertion on a loaded runner), but still heuristic: a >200ms mid-chain stall could in principle read as settled. Accepted; revisit only if the determinism assertion ever flakes in CI.
