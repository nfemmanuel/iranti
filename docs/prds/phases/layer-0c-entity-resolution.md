...
- Close the 4 probed alias misses in the Layer 0b harness without regressing any other metric (or explain every metric that moves).
...
- Bench run showing the 4 alias probes flip, with every other metric delta explained.
...
- [ ] `pnpm bench`: all 4 probed alias cases (`alias:the-figma-file`, `alias:the-reconciliation-doc`, `alias:the-dashboard-run`, `alias:the-sync-wiki-page`) flip from `hit: false` to `hit: true`. `bench/baseline.json` is left untouched (not regenerated) so the run visibly prints the deltas.
