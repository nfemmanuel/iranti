...
- **Stopword sensitivity:** subjects like "the eval branch name" reduce to {eval, branch, name} only if `tokenizeMessage` drops stopwords the same way for keys — verified at build start; if the shared tokenizer keeps short stopwords, containment gets stricter (safer), never looser.
