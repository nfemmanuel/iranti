...
- `src/mcp/tools/aliases.ts`: extend the existing (currently unregistered) module with list/archive handlers as needed.
...
- **D2 — Deactivation writes never target the combined/effective set for scoping the MUTATION — but the guard must accept the effective set for READ-side identification.** Follow `archiveFact`'s exact contract (it takes the effective project ids and archives only in-scope rows): a rule visible to you through combine is yours to deactivate is **rejected** — mutation scope = rules whose `project` ∈ effective read set? No: mirror `archiveFact` precisely, whatever its live semantics are, and pin them with a test. Consistency with the fact surface beats a bespoke rule here; divergence between "which facts can I archive" and "which rules can I deactivate" would be a protocol surprise. (Verified at build start; the test asserts parity between the two.)
...
- **D5 — Alias tools ride along.** Same gap, same shape, library functions already exist (`listAliasesForEntity`, `archiveAlias`); the marginal cost is two thin wrappers + tests, and check 4's caveat showed aliases will need pruning once bare-path learning (AX-9) widens what gets learned. *Rejected:* deferring aliases to "a thin follow-up" again — that deferral is exactly how the gap shipped.
