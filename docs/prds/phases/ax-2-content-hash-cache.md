...
- **§11 content-storage check.** We store `sha256(text)` but never the text. Open: confirm with NF that a one-way hash of message text (not the text itself) is acceptable under master §11's behavioral-data-only constraint. Current read: yes (irreversible, no content recoverable).
