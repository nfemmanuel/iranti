...
- **No content stored beyond extracted slots.** `result` holds only `ExtractedFact[]` (key/value/source/confidence) — the same data already persisted as facts. We do **not** store the raw message text, only its hash. *(§9 §11 question RESOLVED 2026-06-28: §11 governs developer telemetry, not the user's own instance. `extraction_cache` is user-instance data — both the hash and the cached facts are the user's own data, alongside their facts/media — so it is fully allowed. The only invariant: the hash and facts must never enter the telemetry path.)*
...
- **§11 content-storage check — RESOLVED 2026-06-28 (NF).** §11's behavioral-data-only rule constrains **developer telemetry**, not the user's own instance. `extraction_cache` lives in the user's instance (local, and Phase-5 cloud backup) right next to their facts and media — so storing `sha256(rawMessageText)` and the cached `ExtractedFact[]` there is the user's own data and is fully allowed. The hash is also irreversible. **Invariant to preserve:** neither the hash nor the cached facts may ever flow into the telemetry/analytics path (`attend_log` and any future org-collected metric stay behavioural-only). See master §11 "Two data planes, never crossed" and [open-decisions OD-5](../../decisions/open-decisions.md).
...
- 2026-06-28 — §11 question RESOLVED (NF): §11 constrains developer telemetry, not the user's own instance. extraction_cache is user-instance data (hash + cached facts) and is allowed; invariant added that neither may enter the telemetry path. §6 + §9 updated; master §11 gained "Two data planes, never crossed"; open-decisions OD-5 → resolved.
